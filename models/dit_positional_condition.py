...
class ConditionalPositionalEmbeddings(torch.nn.Module):
  """
  Positional embeddings that adapt based on condition input.
  Combines base rotary embeddings with learnable condition-dependent adjustments.
  """
  def __init__(self, dim, base=10_000, cond_dim=None):
    super().__init__()
    self.dim = dim
    self.base = base
    
    # Base rotary embeddings (constant part)
    inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float() / dim))
    self.register_buffer('inv_freq', inv_freq)
    
    # Cached values for base embeddings
    self.seq_len_cached = None
    self.cos_cached = None
    self.sin_cached = None
    
    # Condition-dependent part
    if cond_dim is not None:
      # Project condition to frequency adjustments
      self.cond_freq_proj = nn.Sequential(
        nn.Linear(cond_dim, dim // 2),
        nn.Tanh(),  # Keep adjustments bounded
        nn.Linear(dim // 2, dim // 2)
      )
      
      # Project condition to phase adjustments
      self.cond_phase_proj = nn.Sequential(
        nn.Linear(cond_dim, dim // 2),
        nn.Tanh(),
        nn.Linear(dim // 2, dim // 2)
      )
      
      # Initialize with small weights so initial behavior is close to original
      for module in [self.cond_freq_proj, self.cond_phase_proj]:
        for layer in module:
          if isinstance(layer, nn.Linear):
            nn.init.normal_(layer.weight, std=0.02)
            nn.init.zeros_(layer.bias)
    else:
      self.cond_freq_proj = None
      self.cond_phase_proj = None

  def _compute_base_embeddings(self, seq_len, device, dtype):
    """Compute base rotary embeddings (constant part)"""
    ...
    return self.cos_cached.to(dtype), self.sin_cached.to(dtype)

  def forward(self, x, condition=None, seq_dim=1):
    """
    Args:
      x: input tensor with shape containing sequence dimension
      condition: condition tensor [batch_size, cond_dim] or None
    ...
    # If no condition or no condition projection, return base embeddings
    if condition is None or self.cond_freq_proj is None:
      return cos_base, sin_base
    
    # Compute condition-dependent adjustments
    freq_adj = self.cond_freq_proj(condition)  # [batch_size, dim//2]
    phase_adj = self.cond_phase_proj(condition)  # [batch_size, dim//2]
    
    # Scale adjustments to be small
    freq_adj = freq_adj * 0.1  # Small frequency adjustments
    phase_adj = phase_adj * 0.1  # Small phase adjustments
    
    # Create adjusted frequencies
    t = torch.arange(seq_len, device=device, dtype=dtype)
    
    # Broadcast and apply frequency adjustments
    # freq_adj: [batch_size, dim//2] -> [batch_size, 1, dim//2]
    # inv_freq: [dim//2] -> [1, 1, dim//2]
    adjusted_inv_freq = self.inv_freq[None, None, :] + freq_adj[:, None, :]
    ...
    freqs = freqs + phase_adj[:, None, :]
    
    # Create full embeddings
    emb = torch.cat((freqs, freqs), dim=-1)  # [batch_size, seq_len, dim]
    
    # Convert to cos/sin and add dimensions for qkv and heads
    cos_adj = emb.cos()[:, :, None, None, :].repeat(1, 1, 3, 1, 1)
    sin_adj = emb.sin()[:, :, None, None, :].repeat(1, 1, 3, 1, 1)
    
    # Make transformation on v an identity
    cos_adj[:, :, 2, :, :].fill_(1.)
    sin_adj[:, :, 2, :, :].fill_(0.)
    
    return cos_adj, sin_adj


class Rotary(torch.nn.Module):
  """Legacy rotary embeddings for backward compatibility"""
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int, cond_dim: int = None, *args, **kwargs):
    ...
    if cond_dim is not None:
      ...
      # Store condition dimension for positional embeddings
      self.condition_dim = cond_dim
    else:
      self.cond_embed_prj = None
      self.condition_dim = None
    
    # Use conditional positional embeddings instead of simple rotary
    self.positional_embeddings = ConditionalPositionalEmbeddings(
  ...
  def forward(self, indices, sigma, condition, curr_embed: typing.Optional[torch.Tensor] = None, *args, **kwargs):
    ...
    # Process condition for embedding concatenation
    ...
    # Get conditional positional embeddings
    rotary_cos_sin = self.positional_embeddings(x, condition=condition_for_pos)
